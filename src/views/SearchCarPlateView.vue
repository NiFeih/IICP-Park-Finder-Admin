...
async function handleSearch() {
  ...
  const carPlateQuery = query(
    ...
    where('plateNumber', '==', carPlateNumber.value),
  )
  ...
  if (carPlateQuerySnapshot.empty) {
    ...
    router.push({
      name: 'ownerDetails',
      params: { carPlate: carPlateNumber.value },
    })
  }
}
...
async function fetchStudentDetailsWithPlateNumbers() {
  ...
  return plate_numbers
    ...
    .sort((a, b) => a.plateNumber.localeCompare(b.plateNumber)) // Sort by plate number here
}
const filteredData = computed(() => {
  ...
  if (selectedSearchBy.value === 'Student ID') {
    filteredResults = studentAndPlateNumbers.value.filter(
      data =>
        data.student &&
    ...
    filteredResults = studentAndPlateNumbers.value.filter(data =>
      data.plateNumber.toLowerCase().includes(searchInput.value.toLowerCase()),
    )
  }

  // Sort the filtered results in ascending order by plate number
...
function addToRecentSearches(plate) {
  recentSearches.value = [
    plate,
    ...recentSearches.value.filter(p => p !== plate),
  ].slice(0, 5)
